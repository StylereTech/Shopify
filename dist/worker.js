...
import { PostgresAuditLogRepository, PostgresDeliveryJobRepository, PostgresDispatchAttemptRepository, PostgresIdempotencyRepository, PostgresMerchantConfigRepository } from './infrastructure/persistence/postgres-repositories.js';
...
const orchestrator = new DispatchOrchestrator(merchantConfigs, jobs, idempotency, storreeClient, dispatchAttempts, auditLogs, metrics);
const worker = new RedisDispatchWorker(redis, env.REDIS_QUEUE_KEY, env.REDIS_DEAD_LETTER_KEY, logger, async (payload, attempts) => {
    ...
    }, webhookId, { retryCount: attempts, correlationId });
}, (error) => (error instanceof StorreeDispatchError ? error.retryable : true), metrics, env.WORKER_MAX_RETRIES, 
