...
export class DispatchWatchdog {
    ...
    intervalHandle = null;
    isRunning = false;
    stuckJobState = new Map();
    constructor(pool, jobs, merchantConfigs, storreeClient, dispatchAttempts, auditLogs, metrics, logger, env, maxAttempts = MAX_WATCHDOG_ATTEMPTS) {
        this.pool = pool;
        this.jobs = jobs;
    ...
    async runCycle() {
        ...
        let stuckJobs = [];
        try {
            const { rows } = await this.pool.query(`SELECT * FROM delivery_jobs
    ...
    async handleMaxFailures(job, attempts) {
    ...
    async getShopDomainForMerchant(merchantId) {
        try {
            const { rows } = await this.pool.query('SELECT shop_domain FROM merchant_configs WHERE merchant_id = $1 LIMIT 1', [merchantId]);
...
export function startDispatchWatchdog(pool, jobs, merchantConfigs, storreeClient, dispatchAttempts, auditLogs, metrics, logger, env) {
    const watchdog = new DispatchWatchdog(pool, jobs, merchantConfigs, storreeClient, dispatchAttempts, auditLogs, metrics, logger, env);
    watchdog.start();
    return watchdog;
}
